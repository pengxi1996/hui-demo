  ...
  <div class="h-topbar">
      ...
      <template v-if="!isMergeMenu && menusRoot.length > 1">
        <div class="h-system-list">
          <div class="h-system-item" ref="systemItem">
            <h-menu theme="dark" :active-name="activeIndex" mode="horizontal" ref="toolbarMenu">
              <draggable v-model="menusRoot" @start="drag=true" @end="onEnd" :move="onMove">
                <h-menu-item
                  :name="index"
                  @click.native.prevent.stop="activeSidebar(index,item.id,item.system_code)"
                  v-for="(item, index) in menusRoot"
                  :key="item.index"
                  class="h-topbar-subMenu"
              ...
              <ul>
                <draggable v-model="menusRoot" @start="drag=true" @end="onEnd" :move="onMove">
                  <h-menu-item
                    :name="index"
                    v-for="(item, index) in menusRoot"
                    v-if="index >= showSysLen"
                    :key="item.name"
                    @click.native.prevent.stop="activeSidebar(index,item.id,item.system_code)"
                  >
                    <span
                      class="h-system-title"
    ...
    <div class="h-topbar-tool">
      ...
      <h-menu theme="dark" mode="horizontal">
        ...
        <h-menu-item name="3" v-if="isShowLocalLang && localLangs.length > 0">
          <h-dropdown @on-click="changeLocale">
            ...
            <h-dropdown-menu slot="list">
              <h-dropdown-item
                name="zh-CN"
                :selected="$i18n.locale==='zh-CN'"
                v-if="localLangs.indexOf('zh-CN') != -1"
              >简体中文（中国）</h-dropdown-item>
              <h-dropdown-item
                name="zh-TW"
                :selected="$i18n.locale==='zh-TW'"
                v-if="localLangs.indexOf('zh-TW') != -1"
              >繁体中文（中国台湾）</h-dropdown-item>
              <h-dropdown-item
                name="en-US"
                :selected="$i18n.locale==='en-US'"
                v-if="localLangs.indexOf('en-US') != -1"
              >English</h-dropdown-item>
        ...
        <h-menu-item
          name="4"
          v-if="currTenantId != null && currTenantId != ''&& currTenantId != 'null'"
        >
        ...
        <h-submenu name="7" class="toolLast">
              ...
              <h-row class="h-topbar-tool-personalSkin" v-if="isShowTheme">
                ...
                <h-col span="19">
                  ...
                  <div class="h-topbar-tool-personalSkin-active">
                    <span
                      @click="handleChangeTheme('lightblue')"
                      :title="$t('m.i.topBar.lightblueTheme')"
                      style="border:1px solid rgba(2, 169, 249, 0.4)"
                    >
                      <h-icon
                        name="u-a-skin-default"
                        color="#02A9F9"
                        :class="{'active-theme':this.theme=='lightblue'}"
                      ></h-icon>
                    </span>
                    ...
                    <span
                      @click="handleChangeTheme('black')"
                      :title="$t('m.i.topBar.blackTheme')"
                      style="border:1px solid rgba(0,0,0,0.4)"
                    >
                      <h-icon
                        name="u-a-skin-default"
                        color="#000000"
                        :class="{'active-theme':this.theme=='black'}"
                      ></h-icon>
                    </span>
          ...
          <h-menu-item
            name="7-2"
            class="h-top-perOpt"
            v-if="!isHidePwdAndLockScreen && !freeLogin"
            @click.native="handleResetPass"
          >
    ...
    <h-msg-box
      v-if="!freeLogin"
      v-model="setPassword"
      :title="$t('m.i.common.pwdModify')"
      height="200"
      :closable="closable"
      :mask-closable="false"
      @on-close="handleCancle"
      style="text-align:left"
      class="h-topbar-msgbox"
    >
      ...
      <h-form ref="setPassForm" :model="setPassForm" cols="1" :label-width="116">
        <h-form-item
          prop="oldPassword"
          :label="$t('m.i.common.curPwd')"
          :labelTitle="$t('m.i.common.curPwd')"
          required
        >
          <h-input
            type="password"
            v-model="setPassForm.oldPassword"
            :placeholder="$t('m.i.common.enterCurPwd')"
          ></h-input>
        </h-form-item>
        <h-form-item
          prop="newPassword"
          :label="$t('m.i.common.newPwd')"
          :labelTitle="$t('m.i.common.newPwd')"
          required
        >
          <h-input
            type="password"
            v-model="setPassForm.newPassword"
            :placeholder="$t('m.i.common.enterNewPwd')"
            @on-change="onkeypress"
          ></h-input>
        </h-form-item>
        <h-form-item
          prop="newPasswordCheck"
          :label="$t('m.i.common.newPwdConf')"
          :labelTitle="$t('m.i.common.newPwdConf')"
          required
    ...
    <!-- 修改个人资料 -->
    <h-msg-box
      v-model="changePerspnInfo"
      :title="$t('m.i.topBar.modifyUserInfo')"
      :mask-closable="false"
      width="800"
      class="h-topbar-msgbox"
    >
      <h-form :model="setUser" :label-width="116" ref="setUserModi" cols="2">
        <h-form-item
          :label="$t('m.i.topBar.userName')"
          :labelTitle="$t('m.i.topBar.userName')"
          prop="userName"
          required
        >
        ...
        <!-- 可以选择所属组织 -->
        <h-form-item
          :label="$t('m.i.topBar.relOrg')"
          :labelTitle="$t('m.i.topBar.relOrg')"
          prop="orgId"
          required
        >
        ...
        <h-form-item
          :label="$t('m.i.topBar.mobile')"
          :labelTitle="$t('m.i.topBar.mobile')"
          prop="mobile"
          :validRules="['mobile']"
        >
        ...
        <h-form-item
          :label="$t('m.i.topBar.phone')"
          :labelTitle="$t('m.i.topBar.phone')"
          prop="phone"
          :validRules="['tel']"
        >
        ...
        <h-form-item
          :label="$t('m.i.topBar.fax')"
          :labelTitle="$t('m.i.topBar.fax')"
          prop="fax"
        ...
        <h-form-item
          :label="$t('m.i.topBar.email')"
          :labelTitle="$t('m.i.topBar.email')"
          prop="email"
          :validRules="['email']"
        >
        ...
        <h-form-item
          :label="$t('m.i.topBar.certiType')"
          :labelTitle="$t('m.i.topBar.certiType')"
          prop="cIdentitytype"
        >
          <h-select v-model="setUser.cIdentitytype">
            <h-option
              v-for="item in BIZ_CIDENTTYPE"
              :value="item.code"
              :key="item.code"
            >{{item.text}}</h-option>
          </h-select>
        ...
        <h-form-item
          :label="$t('m.i.topBar.certiNum')"
          :labelTitle="$t('m.i.topBar.certiNum')"
          prop="cIdentityno"
        ...
        <h-form-item
          :label="$t('m.i.topBar.qualiCerti')"
          :labelTitle="$t('m.i.topBar.qualiCerti')"
          prop="cCredentials"
        >
          <h-input v-model="setUser.cCredentials" :maxlength="40"></h-input>
        </h-form-item>
        <h-form-item
          :label="$t('m.i.topBar.workStatus')"
          :labelTitle="$t('m.i.topBar.workStatus')"
          prop="workStatus"
        >
          <h-select v-model="setUser.workStatus">
            <h-option
              v-for="item in SYS_USER_WORK_STATE"
              :value="item.code"
              :key="item.code"
        ...
        <h-form-item
          :label="$t('m.i.topBar.filterMsgType')"
          :labelTitle="$t('m.i.topBar.filterMsgType')"
          prop="msgType"
        >
        ...
        <h-form-item
          :label="$t('m.i.topBar.comments')"
          :labelTitle="$t('m.i.topBar.comments')"
          cols="2"
          prop="remark"
          style="height:72px;"
        >
      ...
      <div slot="footer">
        <h-button
          ...
          style="margin:0 8px"
          @click="userFormReset()"
        >{{$t('m.i.common.reset')}}</h-button>
        <h-button
          type="primary"
          :disabled="submitDisabled"
          @click="setuserSubmit()"
        >{{$t('m.i.common.commit')}}</h-button>
      </div>
    </h-msg-box>
    <!--切换租户-->
    <h-msg-box
      v-model="setTenantModel"
      :title="$t('m.i.topBar.changeTenant')"
      :mask-closable="false"
      width="400"
      height="200"
      @on-ok="submitTenant"
    ...
    >
      <h-form :mode="tenantData" :label-width="116" ref="setTenant" cols="1">
        <h-form-item
          :label="$t('m.i.common.chooseTenant')"
          :labelTitle="$t('m.i.common.chooseTenant')"
          prop="tenantId"
        >
          <h-select v-model="tenantData.tenantId" label-in-value @on-change="selectTenant">
            <h-option
              v-for="item in tenantDataList"
              :value="item.code"
              :key="item.code"
            >{{item.code}}:{{item.text}}</h-option>
          </h-select>
    ...
    <!-- 切换空间 -->
    <h-msg-box
      v-model="setSpaceModel"
      :title="$t('m.i.topBar.changeSpace')"
      :mask-closable="false"
      class="h-topbar-msgbox h-topbar-space"
    >
      <h-tree
        :data="spaceData"
        :show-checkbox="false"
...
import HMsgs from "@frame/components/HMsg";
...
export default {
  data() {
    ...
    let lastLoginDateTime =
      window.sessionStorage.getItem("lastLoginDateTime") || "--"; // 最近登录时间
    ...
    const validatePassCheck = (rule, value, callback) => {
      if (value === "") {
        ...
        callback(
          this.$t("m.i.common.pwdEnterMsg_1") +
            this.minLength +
            this.$t("m.i.common.pwdEnterMsg_2")
        );
      } else if (this.bitCount(this.validatePwd(value)) < this.kindLength) {
        callback(
          this.$t("m.i.common.pwdEnterMsg_3") +
            this.kindLength +
            this.$t("m.i.common.pwdEnterMsg_5")
        );
      } else {
        callback();
      }
      ...
      userfaxRule: [
        {
          test: /^(\d{3,4}-)?\d{7,8}$/,
          trigger: "blur",
          message: this.$t("m.i.common.faxMsg_1")
        }
      ],
      userCIdentitynoRule: [
        {
          test: /^[0-9a-zA-Z]*$/,
          trigger: "blur",
          message: this.$t("m.i.common.cerNumMsg_1")
        }
      ],
      // 数据格式同左侧导航栏菜单格式
      options: [],
  ...
  computed: {
    ...
    logoStyle() {
      ...
      let img = !this.isDefaultLogo
        ? this.tenantLogoImg
        : "./static/logo/${logo}-t.png";
  ...
  watch: {
    ...
    isDragEnded(val) {
      ...
      if (val) {
        ...
        //超出区域不提交数据
        if (
          !obj.hasOwnProperty("placeMenuCode") &&
          !obj.hasOwnProperty("placeKindCode")
        ) {
        ...
        this.$http
          .post(window.LOCAL_CONFIG.OMC_GSV + dropurl, obj)
          .then(res => {
  ...
  methods: {
    changeLocale(locale) {
      ...
      this.$http
        .post(window.LOCAL_CONFIG.OMC_GSV + langUrl, { lang: locale })
        .then(res => {
          if (res && res.data) {
            return;
          }
    ...
    selectTenant(param) {
      ...
      this.tenantData.tenantName =
        tempTenantName && tempTenantName.split(":")[1]
          ? tempTenantName.split(":")[1]
          : param.label;
    },
    submitTenant() {
      let isUrlLowercase = window.LOCAL_CONFIG.isUrlLowercase + "";
      ...
      that.$http
        .post(window.LOCAL_CONFIG.OMC_GSV + tenanturl, tenantObj)
        .then(res => {
          if (res && res.data) {
            ...
            if (code == "0") {
              ...
              window.sessionStorage.setItem(
                "tenantId",
                that.tenantData.tenantId
              );
              window.sessionStorage.setItem(
                "tenantName",
                that.tenantData.tenantName
              );
              Cookies.set("tenantId", that.tenantData.tenantId);
              Cookies.set("tenantName", that.tenantData.tenantName);
              that.$hMessage.success({
                content: that.$t("m.i.topBar.changeTenantMsg_1"),
                duration: 3,
                closable: true
              });
              //切换租户成功后刷新跳转到首页
    ...
    handleSpace() {
      ...
      that.spaceData = [];
      that.$http
        .post(window.LOCAL_CONFIG.OMC_GSV + spaceUrl, obj)
        .then(response => {
    ...
    submitSpace() {
      if (this.spaceNode && this.spaceNode.length == 0) {
        ...
        this.$hMessage.info({
          content:
            this.$t("m.i.topBar.curSpace") +
            "【" +
            name +
            "】，" +
            this.$t("m.i.topBar.changeSpaceMsg_1"),
          duration: 3,
          closable: true
        });
        return false;
        ...
        .then(res => {
          if (res && res.data) {
            ...
            if (code == "0") {
              ...
              that.$hMessage.success({
                content: that.$t("m.i.topBar.changeSpaceMsg"),
                duration: 3,
                closable: true
              });
              //切换空间成功后刷新跳转到首页
    ...
    onEnd(str) {
      let placeNodeInx = this.menusRoot.findIndex(
        item => item.kind_code === this.dragData.placeKindCode
      );
      ...
      this.$http.get(window.LOCAL_CONFIG.OMC_GSV + url).then(res => {
        if (res && res.data) {
          ...
          if (userInfo) {
            ...
            that.setUser.enBusiLineArr =
              that.setUser.enBusiLine.indexOf(",") !== -1
                ? that.setUser.enBusiLine.split(",")
                : [that.setUser.enBusiLine];
            that.setUser.enFieldKindArr =
              that.setUser.enFieldKind.trim().length > 0
                ? that.setUser.enFieldKind.split(",")
                : [];
      ...
      if (this.submitFlag) {
        ...
        this.$refs["setUserModi"].validate(valid => {
          if (valid) {
            ...
            delete this.setUser.childTenantIds;
            this.$http
              .post(window.LOCAL_CONFIG.OMC_GSV + url, this.setUser)
              .then(res => {
    ...
    handleSetPassword() {
      this.$refs["setPassForm"].validate(valid => {
        if (valid) {
          changePwd(this.setPassForm.oldPassword, this.setPassForm.newPassword)
            .then(res => {
    ...
    logOutSys() {
      ...
      let isUseSso = window.LOCAL_CONFIG.isUseSso + "";
      let ssoWeb = window.LOCAL_CONFIG.ssoWeb + "";
      if (isIARCasLogin === "true") {
        this.$store.dispatch("casLogout").then(() => {
          if (isCasAdapter != "true") {
            //非适配器时需要刷新页面重新加载，适配器时通过cas登出后的重定向回跳到首页
            if (isUseSso == "true") {
              //如果开启使用统一运营框架，就跳转到统一运营框架登录页 20200804 modify by zhouzx
              location.href = window.LOCAL_CONFIG.API_HOME + "/" + ssoWeb;
            } else {
              location.reload();
            }
          }
    ...
    activeSidebar(index, id, systemCode) {
      let isUseSso = window.LOCAL_CONFIG.isUseSso + "";
      let currSystem = Cookies.get("currSystem") + ""; //从cookie中获取当前系统编号

      /*如果开启使用统一运营框架，并且不是当前系统时就直接跳转到子系统的首页 20200804 modify by zhouzx*/
      if (
        isUseSso == "true" &&
        systemCode &&
        systemCode != null &&
        ...
        Cookies.set("currSystem", id);
        Cookies.set("activeRootIndex", index);
        location.href = window.LOCAL_CONFIG.API_HOME + "/" + systemCode;
      } else {
        if (index >= 0) {
    ...
    validatePwd(s) {
      ...
      this.tipState =
        this.bitCount(flag) === 1
          ? "weak"
          : this.bitCount(flag) === 2
          ? "general"
          : "complex";
      return flag;
    },
    ...
    handleLeafData(index) {
      ...
      child =
        isMergeMenu === "true"
          ? this.menusRoot
          : this.menusRoot[index] == null
          ? null
          : this.menusRoot[index].children;
    ...
    handleResize() {
      this.$nextTick(() => {
        if (this.$refs.systemItem) {
          this.$store.dispatch(
            "changeSystemHeight",
            this.$refs.systemItem.offsetHeight
          );
          let showSysWidth = document.getElementsByClassName("h-system-list")[0]
            .offsetWidth;
          let subSys = document.getElementsByClassName("h-topbar-subMenu");
          let width = 0;
          for (let i = 0; i < subSys.length; i++) {
            width += subSys[i].offsetWidth ? subSys[i].offsetWidth : 0;
    ...
    handleDicts() {
      ...
      if (isUnderscore === "true") {
        ...
        cidentTypeUrl =
          url +
          "?dict_name=BIZ_CIDENTTYPE" +
          "&lang=" +
          lang +
          "&time=" +
          new Date().getTime();
        this.$http
          .get(window.LOCAL_CONFIG.OMC_GSV + cidentTypeUrl)
          .then(res => {
            let data = res.data instanceof Array ? res.data : [res.data];
        ...
        userWorkStatusUrl =
          url +
          "?dict_name=SYS_USER_WORK_STATE" +
          "&lang=" +
          lang +
          "&time=" +
          new Date().getTime();
        this.$http
          .get(window.LOCAL_CONFIG.OMC_GSV + userWorkStatusUrl)
          .then(res => {
            let data = res.data instanceof Array ? res.data : [res.data];
        ...
        url +=
          "?dict_names=BIZ_CIDENTTYPE,SYS_USER_WORK_STATE,BIZ_MSG_TYPE" +
          "&lang=" +
          lang +
          "&time=" +
          new Date().getTime();
                ...
                if (resp.data && resp.data.result) {
                  ...
                  if (days !== "") {
                    ...
                    if (days > 0 && days <= 3) {
                      ...
                      this.comfirmMsg =
                        days + this.$t("m.i.common.expiredDays");
                    }
                  }
                }
              }
      ...
      checkDefPwd().then(res => {
        if (res) {
          ...
          if (flag === "true") {
            ...
            let isMeetPwdStrength = window.sessionStorage.getItem(
              "isMeetPwdStrength"
            );
    ...
    getTenantLogo() {
      this.$http
        .post(window.LOCAL_CONFIG.OMC_GSV + "/getTenantLogo", {})
        .then(res => {
          if (res && res.data) {
            let logoInfo = res.data instanceof Array ? res.data : [res.data];
            if (
              logoInfo != null &&
              logoInfo.length > 0 &&
              logoInfo[0].logo &&
              ...
              let isShowTenantLogo = window.LOCAL_CONFIG.isShowTenantLogo + "";
              this.isDefaultLogo = isShowTenantLogo == "true" ? false : true;
              this.tenantLogoImg = "data:image/png;base64," + logoInfo[0].logo;
            } else {
    ...
    //多租户是否显示国际化切换按钮
    getTenantIsShowLocalLang() {
      let changeId = Cookies.get("tenantId")
        ? Cookies.get("tenantId")
        : this.currTenantId;
      this.$http
        .post(window.LOCAL_CONFIG.OMC_GSV + "/getTenantByChangeId", {
          changeId: changeId
        })
        .then(res => {
          if (res && res.data) {
            let info = res.data instanceof Array ? res.data : [res.data];
            if (
              info != null &&
              info.length > 0 &&
              info[0].isLocalLang &&
              info[0].isLocalLang == "1"
            ) {
              //租户显示国际化切换按钮
              this.isShowLocalLang = true;
            } else {
              this.isShowLocalLang = false;
  ...
  created() {
    ...
    let isHidePwdAndLockScreen =
      window.LOCAL_CONFIG.isHidePwdAndLockScreen + "";
    ...
    let currSystem = Cookies.get("currSystem");
    let activeRootIndex = Cookies.get("activeRootIndex");
    if (currSystem && currSystem != "") {
      //如果是从其他系统跳转过来的，就选中当前系统 20200807 modify by zhouzx

      this.$nextTick(() => {
        let tmpIndex = Number(activeRootIndex);
        this.activeSidebar(tmpIndex, currSystem, null);
      });
    } else {
      //菜单叶子节点生成下拉数据
    ...
    if (
      window.sessionStorage.getItem("currUserId") &&
      ...
      window.sessionStorage.getItem("currUserId") ==
        Cookies.get("operator_code") &&
      window.sessionStorage.getItem("kindCode") &&
      window.sessionStorage.getItem("envId")
    ) {
    } else {
      window.LOCAL_CONFIG.isToken &&
        getCurrUserInfo().then(res => {
          if (res && res.data) {
            ...
            let orgCode = userInfo.org_code
              ? userInfo.org_code
              : userInfo.orgCode;
            let orgName = userInfo.org_name
              ? userInfo.org_name
              : userInfo.orgName;
            let orgObj = { org_code: orgCode, org_name: orgName };
            window.sessionStorage.setItem("orgList", JSON.stringify(orgObj));
            let lastLoginDateTime = userInfo.last_login_time
            ...
            if (isTenantMode == "true" && this.currUserId != "admin") {
              let tenant_id = userInfo.tenant_id
                ? userInfo.tenant_id
                : userInfo.tenantId;
              if (
                tenant_id == null ||
              ...
              let tenant_name = userInfo.tenant_name
              ...
              let kind_code = userInfo.kind_code
                ? userInfo.kind_code
                : userInfo.kindCode;
              if (
                kind_code == null ||
            ...
            if (this.isShowSpace) {
              ...
              let spaceName = userInfo.env_name
                ? userInfo.env_name
                : userInfo.envName;
    ...
    if (
      isTenantMode === "true" &&
      Cookies.get("operator_code") != "admin" &&
      Cookies.get("tenantId") != ""
    ) {
    ...
    if (
      this.currTenantId != null &&
      this.currTenantId != "" &&
      this.currTenantId != "null"
    ) {
      this.getTenantIsShowLocalLang();
    }
    //查询系统参数，获取锁屏时间
    ...
    if (
      freeLogin == "false" &&
      (isOpenFirstPwd == "true" || isHidePwdAndLockScreen == "false")
    ) {
    ...
    //this.getTenantLogo() 开启多租户时才获取多租户 20200521 modify by zhouzx
  },
  mounted() {
    this.handleResize();
  ...
  beforeCreate() {
    ...
    if (
      ...
      window.sessionStorage.setItem(
        "scrollbarWidth",
        window.LOCAL_CONFIG.TABLE_SCROLLBAR_WIDTH || 7
      );
    }
  }
};
