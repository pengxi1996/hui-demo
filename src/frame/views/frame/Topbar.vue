          ...
          <div class="h-system-item" ref="systemItem">
            <h-menu theme="dark" :active-name="activeIndex" mode="horizontal" ref="toolbarMenu">
              <draggable v-model="menusRoot" @start="drag=true" @end="onEnd" :move="onMove">
                <h-menu-item :name='index' @click.native.prevent.stop="activeSidebar(index,item.id,item.system_code)" v-for="(item, index) in menusRoot" :key="item.index" class="h-topbar-subMenu">{{item.title}}</h-menu-item>
              </draggable>
            </h-menu>
          </div>
        </div>
        <div class="h-system-more-sort" @mouseleave="hideSystemList" v-show="systemHeight>60">
          ...
          <transition name="slide-up">
            <div class="h-system-sort-list" v-show="showSystem && systemHeight>60">
              <ul>
                <draggable v-model="menusRoot" @start="drag=true" @end="onEnd" :move="onMove">
                  <h-menu-item :name='index' v-for="(item, index) in menusRoot" v-if="index >= showSysLen" :key="item.name" @click.native.prevent.stop="activeSidebar(index,item.id,item.system_code)">
...
export default {
  ...
  computed: {
    ...
    logoStyle() {
      ...
      let img=!this.isDefaultLogo?this.tenantLogoImg:'./static/logo/${logo}-t.png'
  ...
  methods: {
      ...
      if (isIARCasLogin === 'true') {
        this.$store.dispatch('casLogout').then(() => {
          if (isCasAdapter != 'true') {
            ...
            if(isUseSso=="true"){//如果开启使用统一运营框架，就跳转到统一运营框架登录页 20200804 modify by zhouzx
    ...
    activeSidebar(index, id,systemCode) {
      ...
      if(isUseSso=="true"&&systemCode&&systemCode!=null&&systemCode!=""&&currSystem!=id){
    ...
    getTenantLogo(){
      this.$http.post(window.LOCAL_CONFIG.OMC_GSV + "/getTenantLogo", {}).then(res => {
          if (res && res.data) {
            let logoInfo=res.data instanceof Array ?res.data:[res.data]
            if(logoInfo!=null&&logoInfo.length>0&&logoInfo[0].logo&&logoInfo[0].logo!=''){
    ...
    getTenantIsShowLocalLang(){
      let changeId = Cookies.get('tenantId') ? Cookies.get('tenantId') : this.currTenantId
      this.$http.post(window.LOCAL_CONFIG.OMC_GSV + "/getTenantByChangeId", {changeId: changeId}).then(res => {
          if (res && res.data) {
            let info=res.data instanceof Array ?res.data:[res.data]
            if(info!=null&&info.length>0&&info[0].isLocalLang&&info[0].isLocalLang=='1'){//租户显示国际化切换按钮
  ...
  created() {
    ...
    if(currSystem&&currSystem!=""){//如果是从其他系统跳转过来的，就选中当前系统 20200807 modify by zhouzx
    ...
    if(this.currTenantId != null && this.currTenantId != '' && this.currTenantId != 'null'){
