...
export default {
  ...
  methods: {
    ...
    closeViewTabs ($event, view, index) {
      ...
      const {
        EMIT_ON_TAB_CLOSE,
        CONFIRM_ON_TAB_CLOSE,
        CONFIRM_MESSAGE_ON_TAB_CLOSE
      } = LOCAL_CONFIG;

      if (
        EMIT_ON_TAB_CLOSE.includes &&
        EMIT_ON_TAB_CLOSE.includes(view.id)
      ) {
        return this.$bus.$emit("close-tab", view.id, onOk);
      }

      if (
        (CONFIRM_ON_TAB_CLOSE.includes &&
          CONFIRM_ON_TAB_CLOSE.includes(view.id)) ||
        (typeof CONFIRM_ON_TAB_CLOSE === "boolean" && CONFIRM_ON_TAB_CLOSE)
      ) {
        this.$hMsgBox.confirm({
          title: CONFIRM_MESSAGE_ON_TAB_CLOSE || "确定要关闭吗？",
          onOk
        });
      } else {
        onOk();
