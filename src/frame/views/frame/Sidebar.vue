  ...
  <div class="h-sidebar-contain" ref="sidebarContain" :class="classObject" @mouseenter="sidebarFocus=true" @mouseleave="sidebarFocus=false">
    <div class="h-sidebar-menu">
      <div class="h-sidebar-menu-home" @click="handleHome" @mouseenter="hideTab(-1,{})">
        <div class="h-sidebar-menu-home-li" :class="{ 'home-active': homeAboard,'is-active':isActive({url:'/mainIndex'}) }">
          <div class="home-item">
      ...
      <div ref="MenuDelayTriangleWrapper" class="h-sidebar-menu-container" @mousemove="onMouseMove" @mouseleave="onMouseLeave">
        <div>
          <ul v-for="(item,sIndex) in childRoute" :key="sIndex" :name="item.id" @mouseenter="onMouseEnter(sIndex, item)">
            <!--只有二级菜单-->
            <div class="h-sidebar-menu-list" v-if="(!item.hasOwnProperty('is_hidden') || item.is_hidden==false || item.is_hidden == 'false') && (item.children == null || item.children.length === 0)">
              <li class="h-sidebar-menu-list-li"  :class="{ 'tab-hover':sIndex==clickItemIndex , 'is-active':isActive(item)}" >
                <div class="list-item" @click="tabActived(sIndex,item.id)">
                  <div class="list-item-menu" @click='openThisMenu($event, item, sIndex)'>
                ...
                <li class="h-sidebar-menu-list-li"  :class="{ 'tab-hover':sIndex==clickItemIndex , 'is-active':isActive(item)}" >
                  <div class="list-item">
                    <div class="list-item-menu">
                      <h-icon class="li-first-i" :name="item.icon || 'u-a-setting'"></h-icon>
                      <span v-if="!item.isShow" class="li-span" :key="sIndex" :title="item.title">{{ item.title }}</span>
                      <span v-else @click="hideTab(sIndex,item)" class="li-span" :key="sIndex+1" :title="item.title">{{ item.title }}</span>
                      <h-icon v-show="collectTotal[item.id] && collectTotal[item.id].length<=0" class="li-last-i" name="xf-arrow-down-right"></h-icon>
                  ...
                  <div class="list-tab">
                    ...
                    <div class="list-tab-panel" :class="classes(item)" :style="styles(sIndex,item.id,item)" @mouseleave="hideTab(sIndex,item, $event)">
                              ...
                              <li class="item" v-for="(subItem, subIndex) in item.children" :key="'b'+subItem.id" :class="{ 'more-fourth-menu-item': (!subItem.hasOwnProperty('is_hidden') || subItem.is_hidden==false || subItem.is_hidden == 'false') && !(subItem.children == null || subItem.children.length === 0),'menu-leaf-item': (!subItem.hasOwnProperty('is_hidden') || subItem.is_hidden==false || subItem.is_hidden == 'false') && ( item.children && (subIndex + 2) <= item.children.length && item.children[subIndex + 1].children !== null && item.children[subIndex + 1].children.length > 0)}">
                                ...
                                <template v-if="(!subItem.hasOwnProperty('is_hidden') || subItem.is_hidden==false || subItem.is_hidden == 'false') && (subItem && subItem.children && subItem.children.length > 0)">
                                  <div class="list-box-nest">
                                    ...
                                    <div class="list-box-menuLeaf" v-for="(box) in subItem.children" :key="'b'+box.id">
                                      <!--只有四级菜单-->
...
import MenuDelayTriangle from "@common/mixins/MenuDelayTriangle.mixin";
...
export default {
  ...
  methods: {
    ...
    isActive (item) {
      ...
      let childrenUrlList = this.menuUrlMap[item.id] || []
      return [item.url].includes(path) || childrenUrlList.includes(path)
    },
    ...
    computeTop (sIndex, id, item) {
      if (this.clickItemIndex == sIndex && !this.firstMountTop[id] && !this.firstMountMaxHeight[id]) {
        ...
        let counts = this.columnsCount(item)
        let hasSearchBox = !!((item && item.children && item.children.length >= 4 && counts.maxFloor > 1))
        ...
        if (!this.firstMountMaxHeight[id]) {
          let maxHeight = window.innerHeight - top - 50 - (hasSearchBox ? 98 : (counts.maxFloor == 1 ? 20 : 40))
    ...
    hideTab (index, item, e) {
      // Preserve the current state when move to sidebar menu from pop-up-panel
      if(e && e.relatedTarget && e.relatedTarget.classList && e.relatedTarget.classList.contains("list-item-menu")) {
        return false;
      }
    ...
    // 生成侧边栏菜单 url list，定位 侧边栏菜单高亮
    initMenuUrlMap () {
      this.menuUrlMap = {}
      this.childRouteData.map((item) => {
        let children = Array.isArray(item.children) ? deepClone(item.children) : []
        let urlList = []
        ...
        this.menuUrlMap[item.id] = urlList
      })
    }
  },
  mounted () {
