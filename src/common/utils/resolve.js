...
/**
 * @description 1.x 的做法，对 menus 做一次初步的处理
 * @param {array} menus - menus from server or local config
 */
export function resolveMenus(menus = []) {
  menus.forEach(menu => {
    if (menu.children && isArray(menu.children) && menu.children.length) {
      return resolveMenus(menu.children);
    }

    if (menu.url) {
      menu._id = uniqueId(); // a internal unique id
      menu.originalUrl = menu.url; // save original url before handle it

      // 菜单 url 相同，ext_field 保存真正的视图组件，路由地址拼接 menu.id 字段以区分
      if (menu.ext_field) {
        menu.url += `_${menu.id}`;
      }
      ...
      if (menu.menu_type === "iframe") {
        menu.url = `/${encodeURIComponent(menu.url + "_" + menu.id)}`; // 避免 url 出现非法字符
      }

      store.state.permission.menusMap.set(
        {
          _id: menu._id,
          url: menu.url
        },
