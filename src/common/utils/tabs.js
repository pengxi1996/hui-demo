...
export default {
  ...
  addNewTab (menuId, url, query = {}, title, menuArgs = '', isKeepAlive, subSysName, compId = '', isIframe = false, meta = {}) {
    ...
    const ctx = this.ctx;
    const { menusMap, tempRoute } = ctx.$store.state.permission;

    let _matched;

    _matched = [...menusMap.keys()].find(key => key.url === url)

    if(_matched) { 
      const { url: path } = menusMap.get(_matched)
      return ctx.$router.push({path, query});
    }

    _matched = tempRoute.find(r => r.id === menuId || r.url === url );

    if(_matched) {
      const { url: path } = _matched;
      return ctx.$router.push({path, query});
    }
    ...
    let menuConfig = {
      ...
      _id: uniqueId() // a internal unique id
    }
    if (isIframe) {
      menuConfig.menu_type = 'iframe'
      menuConfig.originalUrl = url // save original url before handle it
      menuConfig.url = menuConfig.path = url = `/${encodeURIComponent(url)}`; // 避免 url 出现非法字符
    }
