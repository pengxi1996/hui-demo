...
import { getMenuInfoInUcf, isUcf, isFrame, resolveMenus } from "@common/utils";
...
function findMenu (url) {
  const { menusMap, tempRoute } = store.state.permission;
  let matched;

  matched = [...menusMap.keys()].find(key => key.url === url);
  matched = menusMap.get(matched);
...
router.beforeEach((to, from, next) => {
  store.commit("LOAD_ROUTER_VIEW", true); // loading state
  ...
  if (isUcf()) {
    ...
    if (redirectUri.indexOf('operator_code') > 0) {
      ...
      for (let i = 0; i < userinfos.length; i++) {
        ...
        //如果传入了token而不是user_token，表示前端需要自己种token
        if (userInfo.indexOf('token') !== -1 && userInfo.indexOf('user_token') === -1) {
          window.sessionStorage.setItem(
            'token',
            userInfo.substring(userInfo.indexOf('=') + 1, userInfo.length)
          )
          Cookies.set('token', userInfo.substring(userInfo.indexOf('=') + 1, userInfo.length))
        }
...
router.afterEach((to, from) => {
  ...
  store.commit("LOAD_ROUTER_VIEW", false); // finish loading state
})
